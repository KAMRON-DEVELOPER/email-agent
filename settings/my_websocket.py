...
from utils.enums import ChatEvent
...
class WebSocketManager:
    ...
    async def connect(self, websocket: WebSocket, user_id: Optional[str] = None):
        try:
            ...
            if user_id:
                ...
                logger.debug("Anonymous WebSocket connected")
        except Exception as exception:
            logger.exception("Exception while accepting the websocket connection: {exception}")
    ...
    async def disconnect(self, websocket: Optional[WebSocket] = None, user_id: Optional[str] = None):
        try:
            if user_id:
                ...
                logger.debug(f"User with {user_id} ID disconnected")
            elif websocket:
                self.unauthorized_connections.remove(websocket)
                logger.debug("Anonymous WebSocket disconnected")

        except Exception as exception:
            logger.exception(f"Exception while disconnecting the websocket connection: {exception}")
    ...
    async def send_personal_message(self, user_id: str, data: dict):
        ...
        if ws is None:
            logger.warning(f"WebSocket not found for user_id={user_id}. Skipping send.")
            return

        try:
            await ws.send_json(data=data)
        except Exception as exception:
            logger.exception(f"Exception while sending personal message: {exception}")
...
class WebSocketContextManager:
    ...
    async def _disconnect(self):
        if self.pubsub:
            try:
                ...
                logger.exception(f"Failed to close pubsub: {e}")

        try:
            await self.disconnect_handler(self.user_id, self.websocket)
        except Exception as e:
            logger.exception(f"Disconnect handler error: {e}")

        if self.websocket.client_state == WebSocketState.CONNECTED:
            try:
                await self.websocket.close()
            except Exception as e:
                logger.exception(f"Exception while closing WebSocket: {e}")
    ...
    async def _pubsub_listener(self):
        ...
        try:
            async for message in self.pubsub.listen():
                ...
                logger.warning(f"pubsub_data in _pubsub_listener: {pubsub_data}, type: {type(pubsub_data)}")
                ...
                if event_type is None:
                    logger.warning(f"Missing 'type' field in event data: {data}")
                ...
                try:
                    ...
                    logger.exception(f"Invalid event type received: '{event_type}'")
    ...
    async def _websocket_receiver(self):
        ...
        try:
            while self.websocket.client_state == WebSocketState.CONNECTED:
                try:
                    ...
                    try:
                        ...
                        logger.info("Client disconnected on heartbeat")
                        break
                    except Exception as e:
            ...
            logger.debug("WebSocket receiver cancelled")
        except Exception as e:
